...
for i = 11:length(names)%patient2:1-35, patient4:36- ,patient6:
...
wll(i)=mean2(L);
L=glare(L);
L=(normalise(double((L))));
L = (normalise(L)*max_luminosity);
image_lab(:,:,1)=L;
image = lab2rgb(image_lab);
rs2=image(:,:,1);gs2=image(:,:,2);bs2=image(:,:,3);

image=im2double(diff);
image_lab = rgb2lab(image);
max_luminosity = 100;
L = image_lab(:,:,1)./max_luminosity;
pel(i)=mean2(L);
L=glare(L);L=imadjust(L);
 L=(normalise(double((L))));
L = L*max_luminosity;
image_lab(:,:,1)=L;
image = lab2rgb(image_lab);
rd2=image(:,:,1);gd2=image(:,:,2);bd2=image(:,:,3);

image=im2double(perp);
image_lab = rgb2lab(image);
max_luminosity = 100;
L = image_lab(:,:,1)./max_luminosity;
L=glare(L);L=imadjust(L);
...
image_lab(:,:,1)=L;
image = lab2rgb(image_lab);
rp2=image(:,:,1);gp2=image(:,:,2);bp2=image(:,:,3);


% rd = normalise(double(rd)); % red diff
% gd = normalise(double(gd));
% bd = normalise(double(bd));
% rs = normalise(double(rs)); % red sum
% gs = normalise(double(gs));
% bs = normalise(double(bs));
% rp=  normalise(double(rp)); % red perpendicular
% gp=  normalise(double(gp));
% bp=  normalise(double(bp));
% 
...
w=1;
% var11=normalise((double((bp2+gp2+rp2)-(bd2+gd2+rd2))));
% var11=normalise((im2double((bp2+gp2+rp2)-(bd2+gd2+rd2))));
% var11=normalise(im2double((rp2+gp2+bp2)-(rd2+gd2+bd2)));

% wll(i)=mean2(im2double(rs2+gs2+bs2));
% pel(i)=mean2(im2double(rd2+gd2+bd2));
% mpel(i)=mean2(double(-0.0073*rd2-0.0039*gd2+0.1104*bd2));

% var11=normalise(double(imadjust((rp2+gp2+bp2)./3)));


var11=normalise(im2double(imadjust(-0.0073*rd2-0.0039*gd2+0.1104*bd2)));%important
% var11=normalise(im2double(imadjust((bp2+gp2+rp2)-(bd2+gd2+rd2))));


% var11=normalise(im2double((-0.0207*rd2+0.0123*gd2+0.0925*bd2)));
% var11=normalise(im2double((-0.0130*rd2+0.0100*gd2+0.1027*bd2+0.0104*rp2-0.0066*gp2+0.0185*bp2)));

% var11=normalise(im2double(-(-0.0060*rp2-0.0944*gp2+0.0101*bp2)));



%var11=normalise(double(((21.1155*rs2-5.6013*gs2-2.5904*bs2))));
%var11=normalise(im2double(((21.1155*rp2-5.6013*gp2-2.5904*bp2)-(bd2+gd2+rd2))));
% var11=normalise(im2double(imadjust((bp2+gp2+rp2)-(bd2+gd2+rd2),stretchlim(heatmapImage1,[0.01 1-0.01]),[])));

%  var11=normalise(double(((bs2+gs2+rs2))));
%var11=normalise(double((0.0468*bp2-0.0264*gp2+0.0183*rp2)-(0.0058*bd2+0.0006*gd2+0.0019*rd2)));

% var11=normalise((double(((-0.0028*bp2-0.0276*gp2+0.0553*rp2)-(-0.0061*bd2+0.0034*gd2-0.0206*rd2)))));
% var11=normalise((double((-(-0.0061*bd2+0.0034*gd2-0.0206*rd2)))));
% var11=normalise(double(((32.78*rp2-21.28*gp2+3.90*bp2)-(-10.35*rd2+2.91*gd2-1.60*bd2))));
%-(-10.35*rd2+2.91*gd2-1.60*bd2))));
%var11=normalise(im2double(((bs2+gs2+rs2))));
...
%vart(~bwL4)=0;
 ...
 backgroundImage = normalise(imadjust(rs+gs+bs));
...
heatmapImage1 = normalise(double(imgaussfilt(var11,3)));
% heatmapImage1 = var11;
...
%heatmapImage1=(normalise(double((var11))));
...
heatmapImage=(normalise(double((heatmapImage))));
% heatmapImage=(normalise(double((var11))));

% heatmapImage=((im2double(rs+gs+bs)));
...
heatmapImage(g~=0)=0;
% g=imgaussfilt(backgroundImage,30).*g;
% backgroundImage(g~=0)=g(g~=0);
...
% rgb=ind2rgb(uint8(round(heatmapImagec*255)),jet(256));
% imwrite(rgb,'benign_mpel.tif','Compression','none');

heatmapImage(g~=0)=nan;
heatmapImagec= heatmapImage(xi:xm,yi:ym);
...
temp3(bw3)=nan;
    q=q+1;
    L3(q,1)=mean(mean(temp3,'omitnan'),'omitnan');
    %L3(q,2)=std((temp3));
    %L3(q,1)=mean2((temp3));L3(q,2)=std((temp3));
...
%     drawnow;
    ...
    %temp3(bwn2)=0;
    j=j+1;
    L1(j,1)=mean(mean(temp1,'omitnan'),'omitnan');
    %L1(j,2)=std((temp1));
...
gr(j+1:j+q)=3;
Lc=Lc';gr=gr';
figure;boxplot(Lc,gr,'Labels',{'Malignant','Benign','Background'});
ylim([0 1]);
Lt= [Lc(1); Lc(2); Lc(4); Lc(56); Lc(65)];
grt=[1; 1; 1; 2; 2];
figure;boxplot(Lt,grt,'Labels',{'Malignant','Benign'});
ylim([0 1]);
