...
for i = 1:1%27:27%1:length(names)%patient2:1-35, patient4:36- ,patient6:
...
diff_imadjust = diff_lab;

% L = gaussianbpf_dp(L,25,256);
% L = gaussianbpf_dp(L,20,120);
% L = butterworthbpf_caroline(L,10,120,3);
%L = medfilt2(L);
diff_imadjust(:,:,1) = imadjust(L);
L=((L-min(min(L(xmin(t):xmax(t),ymin(t):ymax(t)))))./(max(max(L(xmin(t):xmax(t),ymin(t):ymax(t))))-min(min(L(xmin(t):xmax(t),ymin(t):ymax(t))))))*max_luminosity;
diff_imadjust(:,:,1)=L;
diff_imadjust = lab2rgb(diff_imadjust);

figure; imshow(diff_imadjust);

% diff_histeq = diff_lab;
% diff_histeq(:,:,1) = wiener2(histeq(L))*max_luminosity;
% diff_histeq = lab2rgb(diff_histeq);
% 
% diff_adapthisteq = diff_lab;
% diff_adapthisteq(:,:,1) = wiener2(adapthisteq(L))*max_luminosity;
% diff_adapthisteq = lab2rgb(diff_adapthisteq);





%figure;imshow(sum(xi:xm,yi:ym,:));
...
rd2 = adapthisteq((im2double(glare(rd)))); % red diff
gd2 = adapthisteq((im2double(glare(gd))));
bd2 = adapthisteq((im2double(glare(bd))));
...
% rd3 = bfilter2(rd,w,sigma); % red diff
% gd3 = bfilter2(gd,w,sigma);
% bd3 = bfilter2(bd,w,sigma);
...
w=1;
var11=normalise(im2double(imadjust((21.1155*rp2-5.6013*gp2-2.5904*bp2)-(bd2+gd2+rd2))));
...
% rsc=adapthisteq(rd3(xi:xm,yi:ym));
% gsc=adapthisteq(gd3(xi:xm,yi:ym));
% bsc=adapthisteq(bd3(xi:xm,yi:ym));
